...
router.get('/following/:id', async (req, res, next) => {
  ...
  try {
    const user = await User.findById(userId);
    var following_user = user.following.map(followid => {
      const f_user = await User.findById(followid);
      const {password, email, createdAt, isAdmin,  ...f_userData} = f_user._doc;
      return f_userData;
    });
...
router.get('/followers/:id', async (req, res, next) => {
  ...
  try {
    const user = await User.findById(userId);
    var follower_user = user.followers.map(followid => {
      const f_user = await User.findById(followid);
      const {password, email, createdAt, isAdmin,  ...f_userData} = f_user._doc;
      return f_userData;
    });
